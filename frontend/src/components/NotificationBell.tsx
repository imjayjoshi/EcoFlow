  ...
  return <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="sm" className="relative h-9 w-9 p-0 hover:bg-muted">
          ...
          {unreadCount > 0 && <span className="absolute -right-0.5 -top-0.5 flex h-4.5 w-4.5 items-center justify-center rounded-full bg-status-rejected text-[10px] font-semibold text-white animate-pulse px-[4px] py-[4px]">
      ...
      <PopoverContent align="end" className="w-96 p-0 shadow-xl border-border" sideOffset={8}>
        <div className="flex items-center justify-between border-b border-border px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            {unreadCount > 0 && <span className="rounded-full bg-status-rejected/10 px-2 py-0.5 text-xs font-medium text-status-rejected">
                {unreadCount} new
              </span>}
          </div>
          {unreadCount > 0 && <Button variant="ghost" size="sm" className="h-7 text-xs text-muted-foreground hover:text-foreground" onClick={markAllAsRead}>
              Mark all read
            </Button>}
        </div>
        ...
        <ScrollArea className="h-80">
          <div className="divide-y divide-border">
            {recentNotifications.length === 0 ? <div className="flex flex-col items-center justify-center py-8 text-center">
                <Bell className="h-8 w-8 text-muted-foreground/50 mb-2" />
                <p className="text-sm text-muted-foreground">No notifications</p>
              </div> : recentNotifications.map(notification => {
            const Icon = notificationIcons[notification.type];
            return <div key={notification.id} className={cn("relative flex gap-3 px-4 py-3 transition-colors hover:bg-muted/50 cursor-pointer", !notification.read && "bg-primary/5")} onClick={() => markAsRead(notification.id)}>
                    {/* Priority indicator */}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1.5 flex items-center gap-2">
                        ...
                        {notification.ecoId && <Link to={`/ecos/${notification.ecoId}`} className="text-[10px] font-medium text-primary hover:underline" onClick={e => {
                    e.stopPropagation();
                    setOpen(false);
                  }}>
                            View ECO →
