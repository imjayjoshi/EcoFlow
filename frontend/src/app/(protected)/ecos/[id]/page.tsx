...
export default function ECODetail({ params }: { params: Promise<{ id: string }> }) {
    const { id } = use(params);
    const { user } = useAuth();
    ...
    if (!eco) {
        return (
            <div className="text-center py-12">
                ...
                <Link href="/ecos" className="text-sm text-primary hover:underline mt-2 inline-block">Back to ECOs</Link>
            </div>
        );
    }
