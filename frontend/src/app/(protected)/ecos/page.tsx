            ...
            <div className="flex items-center gap-2">
                ...
                {statuses.map((s) => (
                    <button
                        ...
                        className={`rounded-md px-3 py-1.5 text-xs font-medium transition-colors ${statusFilter === s
                        ...
                        {filtered.map((eco) => (
                            <tr key={eco.id} className="hover:bg-muted/30 transition-colors">
                                <td className="px-5 py-3">
                                    <Link href={`/ecos/${eco.id}`} className="font-mono text-sm text-muted-foreground hover:text-foreground">
                                        {eco.id}
                                    </Link>
                                </td>
                                <td className="px-5 py-3 font-medium text-foreground">
                                    <Link href={`/ecos/${eco.id}`} className="hover:underline">{eco.title}</Link>
                                </td>
                                <td className="px-5 py-3">
